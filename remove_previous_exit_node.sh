...
if ! exit_node_id=$(jq --arg device_hostname "${DEVICE_HOSTNAME:?}" --raw-output '.devices[] | select(.hostname == $device_hostname) | .nodeId' <<< "$tailnet_devices"); then
...
echo "Exit node with device ID '$exit_node_id' has been deleted."
